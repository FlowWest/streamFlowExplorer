...
river_basins <- read_csv(system.file("extdata", "cdec", "cdec_river_basins.csv", 
...
cdec_station_sensor_list_filename <- system.file("extdata", "cdec", "cdec_station_sensor_list.Rds", 
                        package = "streamFlowExplorer", mustWork = TRUE)
...
cdec_stations |> 
  ...
  # st_write("extdata/cdec/cdec_stations.shp", append=FALSE)

# Add manually delineated mainstem identifiers from CSV

all_sections <- read_csv(system.file("extdata", "cdec", "cdec_mainstem_stations.csv", 
                        package = "streamFlowExplorer", mustWork = TRUE)) |>
  janitor::clean_names() |>
  arrange(section) |> pull(section) |> unique()

manual_station_list <- 
  read_csv(system.file("extdata", "cdec", "cdec_mainstem_stations.csv", 
                        package = "streamFlowExplorer", mustWork = TRUE)) |> 
  janitor::clean_names() |>
...
data_avail_by_water_year |>
  ...
  scale_fill_manual(values = c("FALSE" = "white", "TRUE" = "#326875"))
...
if(!file.exists(system.file("extdata", "cdec", "cdec_obs_counts.Rds", 
                        package = "streamFlowExplorer", mustWork = TRUE))) {
  ...
  # obs_counts |> saveRDS("data-raw/cdec_exploration/cdec_obs_counts.Rds")
  
} else {
  
  obs_counts <- readRDS(system.file("extdata", "cdec", "cdec_obs_counts.Rds", 
                        package = "streamFlowExplorer", mustWork = TRUE))
...
# station_table 
# |> write_csv("extdata/cdec/station_table.csv")
...
(`r length(unique(station_table$station_id))` total stations)
...
if(interactive() | coalesce(knitr::pandoc_to(),"")=="html"){
  ...
  leaflet::addPolylines(data=flowlines_wgs84, label=~label, color="#326875", stroke = 2) |>
  leaflet::addCircleMarkers(data=stations_wgs84 |> 
                            ...
                            color = ~if_else(mainstem, "#c1c172", "#888888")) 
} else {
 ggplot() + 
    ...
    scale_color_manual(values = c("TRUE" = "#c1c172", "FALSE" = "#888888"))
}
```
...
**What quality assurance/quality control checks are implemented by monitoring agency?**

CDEC is a secondary data provider. QA/QC is conducted individually by the agencies that collect data. No QA/QC is conducted by CDEC with the exception of applying the following standard error codes:
