...
*sensors indicated are also shown on the map for context but are not included in the summary stats for this report 
...
retrieve_cdec_csv <- function(sta=character(), sen=character(), dur=character(), 
  ...
  filename <- file.path(dir, paste0(name,".csv")) # ".csv.gz"))
  if(!file.exists(filename)){
    ...
    httr::GET(
      ...
      # crunch::write.csv.gz(filename, eol="\r\n") 
      httr::content("raw") |> 
      writeBin(filename)
  } else {
...
count_na_by_wy <- function(filename) {
  read_csv(filename, 
  ...
  obs_counts <- cdec_station_sensors |>
    ...
    mutate(result = pmap(list(station_id, 
                              ...
                              case_when(duration=="event"~"E",
                                        ...
                                        duration=="monthly"~"M"),
                              start, end),
      function(sta,sen,dur,start,end) count_na_by_wy(retrieve_cdec_csv(sta,sen,dur,start,end)))) |>
